...
import { mockFutureRounds, mockSettings, mockPurchaseResponse } from '@/data/mockDemo';
...
const DemoCheckout = () => {
  ...
  const [selectedRounds, setSelectedRounds] = useState<number[]>([mockFutureRounds[0].id]);
  const [copiedPix, setCopiedPix] = useState(false);
  const [copiedCard, setCopiedCard] = useState<string | null>(null);

  const futureRounds = mockFutureRounds.map(r => ({
    id: r.id,
    time: new Date(r.selling_ends_at),
    number: r.number,
    card_price: parseFloat(r.card_price) || 5
  }));

  const purchaseDate = new Date();
  const nextRoundTime = futureRounds.find(r => r.id === selectedRounds[0])?.time || new Date(Date.now() + 8 * 60 * 1000);
  const roundNumber = futureRounds.find(r => r.id === selectedRounds[0])?.number || 1;

  const estimatedPrize = 150 * selectedRounds.length;
  const unitPrice = futureRounds.find(r => r.id === selectedRounds[0])?.card_price || 5;
  const totalPrice = quantity * unitPrice * selectedRounds.length;
  ...
  const handleGeneratePix = () => {
    toast({ title: '✨ Gerando PIX...', description: 'Aguarde um momento' });
    setTimeout(() => {
      setStep('payment');
      ...
      setTimeout(() => {
        setStep('confirmed');
        toast({ title: '🎉 Pagamento confirmado!', description: 'Suas cartelas foram geradas' });
      }, 5000);
    }, 1000);
  };

  const handleCopyPix = () => {
    navigator.clipboard.writeText(mockPurchaseResponse.pix.code);
    setCopiedPix(true);
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {step === 'quantity' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              <Button variant="hero" size="xl" className="w-full" onClick={handleGeneratePix}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              <div className="bg-card border border-border rounded-2xl p-6">
                <div className="text-center mb-6">
                  ...
                  <p className="text-4xl font-bold text-primary">{formatCurrency(totalPrice)}</p>
                  <p className="text-sm text-muted-foreground mt-1">{quantity} cartela{quantity > 1 ? 's' : ''}</p>
                </div>
                <div className="flex justify-center mb-6">
                  <div className="bg-background p-4 rounded-xl">
                    <QRCodeSVG value={mockPurchaseResponse.pix.code} size={200} level="H" includeMargin />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Código Pix (Copia e Cola)</Label>
                  <div className="flex gap-2">
                    <Input value={mockPurchaseResponse.pix.code} readOnly className="font-mono text-xs" />
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center">
                ...
                <p className="text-sm text-muted-foreground">O pagamento será confirmado automaticamente em 5 segundos</p>
              </div>
            </motion.div>
          )}
          ...
          {step === 'confirmed' && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
              ...
              {/* Generated Cards */}
              <div className="bg-card border border-border rounded-2xl p-6">
                <h2 className="font-semibold text-lg mb-4">Suas Cartelas ({mockPurchaseResponse.cards.length})</h2>
                <div className="space-y-3 max-h-60 overflow-y-auto">
                  {mockPurchaseResponse.cards.map((card, index) => (
                    <div key={card.id} className="flex items-center justify-between bg-secondary rounded-xl p-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => navigate('/demo/cartela')}>Abrir</Button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <Button variant="hero" size="xl" className="w-full" onClick={() => navigate('/demo/cartela')}>
